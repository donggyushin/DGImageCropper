...
public final class ImageCropperModel: ObservableObject {
    ...
    public func changeRatio(ratio: CropRatio) {
        ...
        case .width3height4:
            ...
            var w = h / 4 * 3
            var y = 0.0
            var x = originRect.width / 2 - w / 2
            
            /// edgecase
            /// if w is longer than screen width
            if w > originRect.width {
                w = originRect.width
                h = w / 3 * 4
                y = originRect.height / 2 - h / 2
                x = 0
            }
            ...
            bottomTrailingPoint = .init(x: x + w, y: y + h)
        case .width4height3:
            let x = 0.0
    ...
    func move(size: CGSize) {
        ...
        if CheckMoveableRectUseCase(originRect: originRect, rect: GenerateRectUseCase(point1: previousTopLeadingPoint, point2: previousBottomTrailingPoint).execute(), move: size).execute() {
            ...
            if size.width <= 0 && size.height <= 0 {
                ...
                let bottomLeadingPoint = CGPoint(x: bottomTrailingPoint.x - previousRect.width, y: bottomTrailingPoint.y)
                let topTrailingPoint = CGPoint(x: bottomTrailingPoint.x, y: bottomTrailingPoint.y - previousRect.height)
                let topLeadingPoint = CGPoint(x: bottomTrailingPoint.x - previousRect.width, y: bottomTrailingPoint.y - previousRect.height)
